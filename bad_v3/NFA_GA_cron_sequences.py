...
def run_geom_opt(NFA, NFA_str, gen_counter):
    ...
    if exists:
        duplicate_sTDDFT = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sequence/sTDDFT_output/%s.out /ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s/%s.out)' % (file_name, gen_counter, file_name), shell=True)
        duplicate_sTDDFT_prop = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sequence/sTDDFT_output/%s.prop /ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s/%s.prop)' % (file_name, gen_counter, file_name), shell=True)
    ...
    mol.write('xyz', '/ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s/%s.xyz' % (gen_counter, file_name), overwrite=True)

    # copy GFN2 input xyz into GFN2_input directory
    duplicate_xtb_inp = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s/%s.xyz /ihome/ghutchison/blp62/GA/running_GA/sequence/GFN2_input/%s.xyz)' % (gen_counter, file_name, file_name), shell=True)

    # run xTB geometry optimization and sTD-DFT
    calcs = subprocess.call('(cd /ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s && sbatch -J /ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s/%s.xyz /ihome/ghutchison/blp62/GA/running_GA/sequence/xtb_sTDDFT_sequence.slurm)' %(gen_counter, gen_counter, file_name), shell=True)
...
def mutate(NFA, smiles_list, parent_list, parent_b):
    ...
    # change sequence
        ...
        # old mutation
        '''
        # if left terminal unit is donor
        if utils_sequences.even_or_odd(int(new_seq[0])) == 'even':
            # If parent b has no viable donor units, it will reuse unit from parent a
            if len(don_term_units) == 0:
                temp_NFA.append(random.randint(0, len(smiles_list[4])-1))
            else:
                don_term_index = random.randint(0, len(don_term_units)-1)
                temp_NFA.append(don_term_units[don_term_index])
        # if left terminal is acceptor
        else:
            # If parent b has no viable acceptor units, it will reuse unit from parent a
            if len(acc_term_units) == 0:
                temp_NFA.append(random.randint(0, len(smiles_list[0])-1))
            else:
                acc_term_index = random.randint(0, len(acc_term_units)-1)
                temp_NFA.append(acc_term_units[acc_term_index])
        ...
        if utils_sequences.even_or_odd(int(new_seq[1])) == 'even':
            ...
            if len(don_core_units) == 0:
                ...
                don_core_index = random.randint(0, len(don_core_units)-1)
                temp_NFA.append(don_core_units[don_core_index])
        else:
            # If parent b has no viable acceptor units, it will reuse unit from parent a
            if len(acc_core_units) == 0:
                temp_NFA.append(random.randint(0, len(smiles_list[2])-1))
            else:
                acc_core_index = random.randint(0, len(acc_core_units)-1)
                temp_NFA.append(acc_core_units[acc_core_index])
        ...
        if utils_sequences.even_or_odd(int(new_seq[4])) == 'even':
            # If parent b has no viable donor units, it will reuse unit from parent a
            if len(don_term_units) == 0:
                temp_NFA.append(random.randint(0, len(smiles_list[5])-1))
            else:
                don_term_index = random.randint(0, len(don_term_units)-1)
                temp_NFA.append(don_term_units[don_term_index])
        # if left terminal is acceptor
            ...
            # If parent b has no viable acceptor units, it will reuse unit from parent a
            if len(acc_term_units) == 0:
                temp_NFA.append(random.randint(0, len(smiles_list[3])-1))
            else:
                acc_term_index = random.randint(0, len(acc_term_units)-1)
                temp_NFA.append(acc_term_units[acc_term_index])
        '''
...
def crossover_mutate(parent_list, smiles_list, current_pop_str):
   ...
   # create backup lists of units from parent a
    ...
    # start fresh. Try again with crossover
    ...
    if int(seq_list[4]) == int(seq_list[0]):
        ...
        # if new acceptor
            ...
            temp_child.append(temp_unit)
            seq_type.append('odd')
    # starts old crossover

    # create new child, with sequence of parent a and units of parent b (if not enough though, use parent a units)
    
    # if left terminal unit is donor
    '''
    if utils_sequences.even_or_odd(int(seq_list[0])) == 'even':
        # If parent b has no viable donor units, it will reuse unit from parent a  
        if track_don_count < don_term_units:
            don_term_index = random.randint(0, len(don_term_units)-1)
            temp_child.append(don_term_units[don_term_index])
            track_don_count.append(don_term_units[don_term_index])
        else:
            temp_child.append(parent_list[parent_a][0])
            track_don_count.append(parent_list[parent_a][0])
    ...
    # if left terminal is acceptor
        ...
        if len(acc_term_units) == 0:
            ...
            acc_term_index = random.randint(0, len(acc_term_units)-1)
            temp_child.append(acc_term_units[acc_term_index])
    ...
    if utils_sequences.even_or_odd(int(seq_list[1])) == 'even':
        ...
        if len(don_core_units) == 0:
            ...
            don_core_index = random.randint(0, len(don_core_units)-1)
            temp_child.append(don_core_units[don_core_index])
    else:
        # If parent b has no viable acceptor units, it will reuse unit from parent a
        if len(acc_core_units) == 0:
            temp_child.append(parent_list[parent_a][1])
            ...
            acc_core_index = random.randint(0, len(acc_core_units)-1)
            temp_child.append(acc_core_units[acc_core_index])
    ...
    if utils_sequences.even_or_odd(int(seq_list[3])) == 'even':
        # If parent b has no viable donor units, it will reuse unit from parent a
        if len(don_core_units) == 0:
            temp_child.append(parent_list[parent_a][3])
        else:
            don_core_index = random.randint(0, len(don_core_units)-1)
            temp_child.append(don_core_units[don_core_index])
    else:
    ...
    if utils_sequences.even_or_odd(int(seq_list[4])) == 'even':
        # If parent b has no viable donor units, it will reuse unit from parent a
        if len(don_term_units) == 0:
            temp_child.append(parent_list[parent_a][4])
        else:
            don_term_index = random.randint(0, len(don_term_units)-1)
            temp_child.append(don_term_units[don_term_index])
    # if left terminal is acceptor
...
def init_gen(pop_size, gen_counter, smiles_list):
    ...
    setup = subprocess.call('mkdir /ihome/ghutchison/blp62/GA/running_GA/sequence/opt_xyz /ihome/ghutchison/blp62/GA/running_GA/sequence/GFN2_input /ihome/ghutchison/blp62/GA/running_GA/sequence/GFN2_output /ihome/ghutchison/blp62/GA/running_GA/sequence/generations /ihome/ghutchison/blp62/GA/running_GA/sequence/sTDDFT_output /ihome/ghutchison/blp62/GA/running_GA/sequence/sTDDFT_input', shell=True)

    # create directory for generation 1
    gen1 = subprocess.call('mkdir /ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s' % (gen_counter), shell=True)
    ...
    with open('/ihome/ghutchison/blp62/GA/running_GA/sequence/quick_analysis_data.csv', 'w') as quick_file:
    ...
    with open('/ihome/ghutchison/blp62/GA/running_GA/sequence/full_analysis_data.csv', 'w') as analysis_file:
        # write analysis file headers
...
def evaluate_prev_gen(gen_counter):
    ...
    remove_test = subprocess.call('(cd /ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s && rm /ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s/test.out)' % (prev_gen, prev_gen), shell=True)
    
    for output in glob.iglob('/ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s/*.out' % (prev_gen)):
        
        NFA_filename = output.split('/')[-1].split('.')[0]

        orig_smi = utils_sequences.xyz_to_smiles('/ihome/ghutchison/blp62/GA/running_GA/sequence/GFN2_input/%s.xyz' % (NFA_filename))
        # check if geom is good. If not, do not evaluate and pass to next generation
        if utils_sequences.check_geom_opt(orig_smi, '/ihome/ghutchison/blp62/GA/running_GA/sequence/opt_xyz/%s.xyz'% (NFA_filename)) == False:
            copy_badgeom = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sequence/opt_xyz/%s.xyz /ihome/ghutchison/blp62/GA/running_GA/sequence/bad_geom_xyz/%s.xyz)' % (NFA_filename, NFA_filename), shell=True)
    ...
    with open('/ihome/ghutchison/blp62/GA/running_GA/sequence/quick_analysis_data.csv', 'a') as quick_file:
    ...
    for x in range(len(fitness_list[0])):
        ...
        with open('/ihome/ghutchison/blp62/GA/running_GA/sequence/full_analysis_data.csv', 'a') as analysis_file:
            analysis_file.write('%d,%s,%f,%f,%f,%s,\n' % (prev_gen, file_name, deltaHOMO, summed_oscs, PCE, donor))

    # make backup copies of output files
    shutil.copy('/ihome/ghutchison/blp62/GA/running_GA/sequence/quick_analysis_data.csv', '/ihome/ghutchison/blp62/GA/running_GA/sequence/quick_analysis_data_copy.csv')
    shutil.copy('/ihome/ghutchison/blp62/GA/running_GA/sequence/full_analysis_data.csv', '/ihome/ghutchison/blp62/GA/running_GA/sequence/full_analysis_data_copy.csv')
...
def next_gen(gen_counter, pop_size, smiles_list, fitness_list):
    ...
    setup = subprocess.call('mkdir /ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s' % (gen_counter), shell=True)
...
def main(gen_counter):
    ...
    if gen_counter == 1:
        ...
        rand_file = open('/ihome/ghutchison/blp62/GA/running_GA/sequence/rand_states/randstate_%s.p' % (str(gen_counter)), 'wb')
    ...
    # run if NOT the initial generation
        ...
        open_rand = open('/ihome/ghutchison/blp62/GA/running_GA/sequence/rand_states/randstate_%s.p' % (prev_gen), 'rb')
...
def check_prev_gen_complete(gen_counter):
    ...
    if gen_counter == 1:
        ...
        prop_counter = 0
        for x in glob.iglob('/ihome/ghutchison/blp62/GA/running_GA/sequence/generations/%s/*.prop' % (prev_gen)):
            prop_counter +=1
...
if __name__ == '__main__':
    ...
    rootdir = '/ihome/ghutchison/blp62/GA/running_GA/sequence/generations/'
    gen_counter = 0
