...
def run_geom_opt(NFA, NFA_str, generation_counter):
    ...
    exists = os.path.isfile('/ihome/ghutchison/blp62/GA/running_GA/opt_xyz/%s.xyz' % (file_name))
    if exists:
    ...
    mol.write('xyz', '/ihome/ghutchison/blp62/GA/running_GA/GFN2_input/%s.xyz' % (file_name), overwrite=True)

    # make directory to run xtb in for the NFA
    mkdir_NFA = subprocess.call('(mkdir /ihome/ghutchison/blp62/GA/running_GA/GFN2_input/%s)' % (file_name), shell=True)

    # run xTB geometry optimization
    xtb = subprocess.call('(cd /ihome/ghutchison/blp62/GA/running_GA/GFN2_input/%s && /ihome/ghutchison/geoffh/xtb/xtb /ihome/ghutchison/blp62/GA/running_GA/GFN2_input/%s.xyz --opt > /ihome/ghutchison/blp62/GA/running_GA/GFN2_input/%s/%s.out)' %
                          (file_name, file_name, file_name, file_name), shell=True)

    save_opt_file = subprocess.call(
        '(cp /ihome/ghutchison/blp62/GA/running_GA/GFN2_input/%s/xtbopt.xyz /ihome/ghutchison/blp62/GA/running_GA/opt_xyz/%s.xyz)' % (file_name, file_name), shell=True)

    # copy GFN2 output file ints GFN2_output directory
    save_GFN2_file = subprocess.call(
        '(cp /ihome/ghutchison/blp62/GA/running_GA/GFN2_input/%s/%s.out /ihome/ghutchison/blp62/GA/running_GA/GFN2_output/%s.out)' % (file_name, file_name, file_name), shell=True)
...
def init_gen(pop_size, gen_counter, smiles_list):
    ...
    NFAs = []
    for output in glob.iglob('/ihome/ghutchison/blp62/GA/running_GA/generations/1/*.inp'):
