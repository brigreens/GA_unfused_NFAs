...
def run_geom_opt(NFA, NFA_str, gen_counter):
    ...
    if exists:
        duplicate_sTDDFT = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sTDDFT_output/%s.out /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/%s.out)' % (file_name, gen_counter, file_name), shell=True)
        duplicate_sTDDFT_prop = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sTDDFT_output/%s.prop /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/%s.prop)' % (file_name, gen_counter, file_name), shell=True)
        ...
        mol.write('xyz', '/ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/%s.xyz' % (gen_counter, file_name), overwrite=True)

        # copy GFN2 input xyz into GFN2_input directory
        duplicate_xtb_inp = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/%s.xyz /ihome/ghutchison/blp62/GA/running_GA/more_units/GFN2_input/%s.xyz)' % (gen_counter, file_name, file_name), shell=True)

        # run xTB geometry optimization and sTD-DFT
        calcs = subprocess.call('(cd /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s && sbatch -J /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/%s.xyz /ihome/ghutchison/blp62/GA/running_GA/more_units/xtb_sTDDFT_moreunits.slurm)' %(gen_counter, gen_counter, file_name), shell=True)
...
def run_sTDDFT(gen_counter, NFA):
    ...
    if exists:
        ...
        create_input = subprocess.call('(cd /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s && obabel /ihome/ghutchison/blp62/GA/running_GA/opt_xyz/%s.xyz -O .inp -o orcainp -m -xf /ihome/ghutchison/blp62/GA/running_GA/cam_b3lyp_def2_SVP_pp.txt)' % (gen_counter, file_name), shell=True)
        # submit sTD-DFT calculation
        sTDDFT_submit = subprocess.call('(cd /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s && sbatch -J %s.inp /ihome/ghutchison/blp62/orca4_sTDDFT_pp.slurm)' % (gen_counter, file_name), shell=True)
...
def move_files(file_name, prev_gen):
    ...
    copy_sTDDFT_output = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/%s.out /ihome/ghutchison/blp62/GA/running_GA/sTDDFT_output/%s.out)' % (prev_gen, file_name, file_name), shell=True)
    copy_sTDDFT_prop = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/%s.prop /ihome/ghutchison/blp62/GA/running_GA/sTDDFT_output/%s.prop)' % (prev_gen, file_name, file_name), shell=True)
...
def init_gen(pop_size, gen_counter, smiles_list):
    ...
    setup = subprocess.call('mkdir /ihome/ghutchison/blp62/GA/running_GA/more_units/opt_xyz /ihome/ghutchison/blp62/GA/running_GA/more_units/GFN2_input /ihome/ghutchison/blp62/GA/running_GA/more_units/GFN2_output /ihome/ghutchison/blp62/GA/running_GA/more_units/generations /ihome/ghutchison/blp62/GA/running_GA/more_units/sTDDFT_output /ihome/ghutchison/blp62/GA/running_GA/more_units/sTDDFT_input', shell=True)

    # create directory for generation 1
    gen1 = subprocess.call('mkdir /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s' % (gen_counter), shell=True)
    ...
    with open('/ihome/ghutchison/blp62/GA/running_GA/more_units/quick_analysis_data.csv', 'w') as quick_file:
    ...
    with open('/ihome/ghutchison/blp62/GA/running_GA/more_units/full_analysis_data.csv', 'w') as analysis_file:
        # write analysis file headers
...
def evaluate_prev_gen(gen_counter):
    ...
    remove_test = subprocess.call('(cd /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s && rm /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/test.out)' % (prev_gen, prev_gen), shell=True)
    
    for output in glob.iglob('/ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/*.out' % (prev_gen)):
        
        NFA_filename = output.split('/')[-1].split('.')[0]

        orig_smi = utils.xyz_to_smiles('/ihome/ghutchison/blp62/GA/running_GA/more_units/GFN2_input/%s.xyz' % (NFA_filename))
        # check if geom is good. If not, do not evaluate and pass to next generation
        if utils.check_geom_opt(orig_smi, '/ihome/ghutchison/blp62/GA/running_GA/more_units/opt_xyz/%s.xyz'% (NFA_filename)) == False:
            copy_badgeom = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/more_units/opt_xyz/%s.xyz /ihome/ghutchison/blp62/GA/running_GA/more_units/bad_geom_xyz/%s.xyz)' % (NFA_filename, NFA_filename), shell=True)
            pass
        else:
    ...
    with open('/ihome/ghutchison/blp62/GA/running_GA/more_units/quick_analysis_data.csv', 'a') as quick_file:
    ...
    for x in range(len(fitness_list[0])):
        ...
        with open('/ihome/ghutchison/blp62/GA/running_GA/more_units/full_analysis_data.csv', 'a') as analysis_file:
            analysis_file.write('%d,%s,%f,%f,%f,%s,\n' % (prev_gen, file_name, deltaHOMO, summed_oscs, PCE, donor))

    # make backup copies of output files
    shutil.copy('/ihome/ghutchison/blp62/GA/running_GA/more_units/quick_analysis_data.csv', '/ihome/ghutchison/blp62/GA/running_GA/more_units/quick_analysis_data_copy.csv')
    shutil.copy('/ihome/ghutchison/blp62/GA/running_GA/more_units/full_analysis_data.csv', '/ihome/ghutchison/blp62/GA/running_GA/more_units/full_analysis_data_copy.csv')
...
def next_gen(gen_counter, pop_size, smiles_list, fitness_list):
    ...
    setup = subprocess.call('mkdir /ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s' % (gen_counter), shell=True)
...
def main(gen_counter):
    ...
    if gen_counter == 1:
        ...
        rand_file = open('/ihome/ghutchison/blp62/GA/running_GA/more_units/rand_states/randstate_%s.p' % (str(gen_counter)), 'wb')
    ...
    # run if NOT the initial generation
        ...
        open_rand = open('/ihome/ghutchison/blp62/GA/running_GA/more_units/rand_states/randstate_%s.p' % (prev_gen), 'rb')
...
def check_prev_gen_complete(gen_counter):
    ...
    if gen_counter == 1:
        ...
        prop_counter = 0
        for x in glob.iglob('/ihome/ghutchison/blp62/GA/running_GA/more_units/generations/%s/*.prop' % (prev_gen)):
            prop_counter +=1
...
if __name__ == '__main__':
    ...
    rootdir = '/ihome/ghutchison/blp62/GA/running_GA/more_units/generations/'
    gen_counter = 0
