...
def run_geom_opt(NFA, NFA_str, gen_counter):
    ...
    file_name = utils_final.make_file_name(NFA)
    ...
    exists = os.path.isfile('/ihome/ghutchison/blp62/GA/running_GA/sequence/sTDDFT_output/%s.out' % (file_name))
    if exists:
        duplicate_sTDDFT = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sequence/sTDDFT_output/%s.out /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/%s.out)' % (file_name, gen_counter, file_name), shell=True)
        duplicate_sTDDFT_prop = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sequence/sTDDFT_output/%s.prop /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/%s.prop)' % (file_name, gen_counter, file_name), shell=True)
    ...
    utils_final.make3D(mol)

    # write NFA .xyz file to containing folder
    mol.write('xyz', '/ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/%s.xyz' % (gen_counter, file_name), overwrite=True)

    # copy GFN2 input xyz into GFN2_input directory
    duplicate_xtb_inp = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/%s.xyz /ihome/ghutchison/blp62/GA/running_GA/opt_GA/GFN2_input/%s.xyz)' % (gen_counter, file_name, file_name), shell=True)

    # run xTB geometry optimization and sTD-DFT
    calcs = subprocess.call('(cd /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s && sbatch -J /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/%s.xyz /ihome/ghutchison/blp62/GA/running_GA/opt_GA/xtb_sTDDFT_optGA.slurm)' %(gen_counter, gen_counter, file_name), shell=True)
...
def find_elec_prop(file_names):
    ...
    for NFA in file_names:
        ...
        stddft_props = utils_final.parse_sTDDFT(NFA)
...
def mutate(NFA, smiles_list, parent_list, parent_b):
    ...
    if type == 0:
        ...
        if point == 0:
            if utils_final.even_or_odd(int(seq[0])) == 'even':
        ...
        # replace core 1
            ...
            if utils_final.even_or_odd(int(seq[1])) == 'even':
                # donor core
                new_unit = random.randint(0, len(smiles_list[1]) - 1)
            else:
                # acceptor core
        ...
        # replace core 2
            ...
            if utils_final.even_or_odd(int(seq[2])) == 'even':
                # donor core
                new_unit = random.randint(0, len(smiles_list[1]) - 1)
            else:
                # acceptor core
        ...
        # replace core 3
            ...
            if utils_final.even_or_odd(int(seq[3])) == 'even':
                # donor core
                new_unit = random.randint(0, len(smiles_list[1]) - 1)
            else:
                # acceptor core
        ...
        # replace right terminal
            ...
            if utils_final.even_or_odd(int(seq[4])) == 'even':
                # donor core
                new_unit = random.randint(0, len(smiles_list[1]) - 1)
            else:
                # acceptor core
    ...
    # change sequence
        ...
        if utils_final.even_or_odd(int(orig_seq[0])) == 'even':
            don_core_units.append(parent_list[parent_b][0])
        else:
            acc_term_units.append(parent_list[parent_b][0])

        if utils_final.even_or_odd(int(orig_seq[1])) == 'even':
            if parent_list[parent_b][1] not in don_core_units:
        ...
        if utils_final.even_or_odd(int(orig_seq[2])) == 'even':
            if parent_list[parent_b][2] not in don_core_units:
        ...
        if utils_final.even_or_odd(int(orig_seq[3])) == 'even':
            if parent_list[parent_b][3] not in don_core_units:
        ...
        if utils_final.even_or_odd(int(orig_seq[4])) == 'even':
            if parent_list[parent_b][4] not in don_core_units:
                don_core_units.append(parent_list[parent_b][4])
        else:
        ...
        new_seq_str = str(utils_final.select_seq())
        ...
        # new mutation
        # left terminal unit
        if utils_final.even_or_odd(int(new_seq[0])) == 'even':
            if len(don_core_units) != 0:
                temp_NFA.append(don_core_units[0])
                don_core_count +=1
            else:
                temp_NFA.append(random.randint(0, len(smiles_list[1])-1))
        ...
        if int(new_seq[1]) == int(new_seq[0]):
            if utils_final.even_or_odd(int(new_seq[0])) == 'even':
                ...
                seq_type.append('odd')
                # 228 is the highest index of acceptor cores (so as to make sure a terminal is not picked as a core)
                if temp_NFA[0] <= 67:
                    temp_NFA.append(temp_NFA[0])
                else:
                    core1 = random.randint(0, len(smiles_list[2])-1)
                    temp_NFA.append(core1)
                    new_seq_str = utils_final.update_seq(new_seq_str, 'core1', 'acc')
                    new_seq = list(new_seq_str)

        else:
            # if new donor
            if utils_final.even_or_odd(int(new_seq[1])) == 'even':
        ...
        if int(new_seq[2]) == int(new_seq[0]):
            if utils_final.even_or_odd(int(new_seq[0])) == 'even':
                ...
                if temp_NFA[0] <= 67:
                    ...
                    new_seq_str = utils_final.update_seq(new_seq_str, 'core2', 'acc')
                    new_seq = list(new_seq_str)
            ...
            # if new donor
            if utils_final.even_or_odd(int(new_seq[2])) == 'even':
                # if there is enough donor units
                if len(don_core_units) > don_core_count:
                    temp_unit = don_core_units[don_core_count]
                    don_core_count +=1
        ...
        if int(new_seq[3]) == int(new_seq[0]):
            if utils_final.even_or_odd(int(new_seq[0])) == 'even':
                ...
                if temp_NFA[0] <= 67:
                    ...
                    new_seq_str = utils_final.update_seq(new_seq_str, 'core3', 'acc')
                    new_seq = list(new_seq_str)
            ...
            # if new donor
            if utils_final.even_or_odd(int(new_seq[3])) == 'even':
                # if there is enough donor units
                if len(don_core_units) > don_core_count:
                    temp_unit = don_core_units[don_core_count]
                    don_core_count +=1
        ...
        if int(new_seq[4]) == int(new_seq[0]):
            ...
            # if new donor
            if utils_final.even_or_odd(int(new_seq[4])) == 'even':
                # if there is enough donor units
                if len(don_core_units) > don_core_count:
                    temp_unit = don_core_units[don_core_count]
                    don_core_count +=1
...
def crossover_mutate(parent_list, smiles_list, current_pop_str):
    ...
    acc_units = []
    for x in seq_list:
        if utils_final.even_or_odd(x) == 'even':
    ...
    if utils_final.even_or_odd(int(temp_seq_list[0])) == 'even':
        don_core_units.append(parent_list[parent_b][0])
    else:
        acc_term_units.append(parent_list[parent_b][0])

    if utils_final.even_or_odd(int(temp_seq_list[1])) == 'even':
        if parent_list[parent_b][1] not in don_core_units:
            don_core_units.append(parent_list[parent_b][1])
    else:
        if parent_list[parent_b][1] not in acc_term_units:
    ...
    if utils_final.even_or_odd(int(temp_seq_list[2])) == 'even':
        if parent_list[parent_b][2] not in don_core_units:
            don_core_units.append(parent_list[parent_b][2])
    else:
        if parent_list[parent_b][2] not in acc_term_units:
    ...
    if utils_final.even_or_odd(int(temp_seq_list[3])) == 'even':
        if parent_list[parent_b][3] not in don_core_units:
            don_core_units.append(parent_list[parent_b][3])
    else:
        if parent_list[parent_b][3] not in acc_term_units:
    ...
    if utils_final.even_or_odd(int(temp_seq_list[4])) == 'even':
        if parent_list[parent_b][4] not in don_core_units:
            don_core_units.append(parent_list[parent_b][4])
    else:
        if parent_list[parent_b][4] not in acc_term_units:
   ...
   # create backup lists of units from parent a
    ...
    if utils_final.even_or_odd(int(seq_list[0])) == 'even':
        backup_don_core_units.append(parent_list[parent_a][0])
    else:
        backup_acc_term_units.append(parent_list[parent_a][0])

    if utils_final.even_or_odd(int(seq_list[1])) == 'even':
        if parent_list[parent_a][1] not in backup_don_core_units:
    ...
    if utils_final.even_or_odd(int(seq_list[2])) == 'even':
        if parent_list[parent_a][2] not in backup_don_core_units:
    ...
    if utils_final.even_or_odd(int(seq_list[3])) == 'even':
        if parent_list[parent_a][3] not in backup_don_core_units:
    ...
    if utils_final.even_or_odd(int(seq_list[4])) == 'even':
        if parent_list[parent_a][4] not in backup_don_core_units:
            backup_don_core_units.append(parent_list[parent_a][4])
    else:
    ...
    if utils_final.even_or_odd(int(seq_list[0])) == 'even':
        if len(don_core_units) != 0:
            temp_unit = don_core_units[0]
            don_core_count +=1
        else:
            temp_unit = backup_don_core_units[0]
    ...
    if int(seq_list[1]) == int(seq_list[0]):
        ...
        if utils_final.even_or_odd(int(seq_list[0])) == 'even':
            ...
            if temp_child[0] <= 67:
                ...
                seq = utils_final.update_seq(seq, 'core1', 'acc')
                seq_list = list(str(seq))
    ...
    if int(seq_list[2]) == int(seq_list[0]):
        ...
        if utils_final.even_or_odd(int(seq_list[0])) == 'even':
            ...
            if temp_child[0] <= 67:
                ...
                seq = utils_final.update_seq(seq, 'core2', 'acc')
                seq_list = list(str(seq))
    ...
    if int(seq_list[3]) == int(seq_list[0]):
        ...
        if utils_final.even_or_odd(int(seq_list[0])) == 'even':
            ...
            if temp_child[0] <= 67:
                ...
                seq = utils_final.update_seq(seq, 'core3', 'acc')
                seq_list = list(str(seq))
    ...
    temp_child_str = utils_final.make_NFA_str(temp_child, smiles_list)
...
def init_gen(pop_size, gen_counter, smiles_list):
    ...
    setup = subprocess.call('mkdir /ihome/ghutchison/blp62/GA/running_GA/opt_GA/opt_xyz /ihome/ghutchison/blp62/GA/running_GA/opt_GA/GFN2_input /ihome/ghutchison/blp62/GA/running_GA/opt_GA/GFN2_output /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations /ihome/ghutchison/blp62/GA/running_GA/opt_GA/sTDDFT_output /ihome/ghutchison/blp62/GA/running_GA/opt_GA/sTDDFT_input', shell=True)

    # create directory for generation 1
    gen1 = subprocess.call('mkdir /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s' % (gen_counter), shell=True)
    ...
    while counter < pop_size:
        ...
        sequence = utils_final.select_seq()

        # temp_NFA = [L_term_index, core1_index, core2_index, core3_index, R_term_index, sequence]
        temp_NFA = utils_final.make_seq_NFA(sequence, smiles_list)

        # make SMILES string of NFA
        temp_NFA_str = utils_final.make_NFA_str(temp_NFA, smiles_list)
    ...
    with open('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/quick_analysis_data.csv', 'w') as quick_file:
    ...
    with open('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/full_analysis_data.csv', 'w') as analysis_file:
        # write analysis file headers
...
def evaluate_prev_gen(gen_counter):
    ...
    remove_test = subprocess.call('(cd /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s && rm /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/test.out)' % (prev_gen, prev_gen), shell=True)
    
    for output in glob.iglob('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/*.out' % (prev_gen)):
        
        NFA_filename = output.split('/')[-1].split('.')[0]

        orig_smi = utils_final.xyz_to_smiles('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/GFN2_input/%s.xyz' % (NFA_filename))
        # check if geom is good. If not, do not evaluate and pass to next generation
        if utils_final.check_geom_opt(orig_smi, '/ihome/ghutchison/blp62/GA/running_GA/opt_GA/opt_xyz/%s.xyz'% (NFA_filename)) == False:
            copy_badgeom = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/opt_GA/opt_xyz/%s.xyz /ihome/ghutchison/blp62/GA/running_GA/opt_GA/bad_geom_xyz/%s.xyz)' % (NFA_filename, NFA_filename), shell=True)
            
            pass
        elif utils_final.check_orca(output) == False:
            pass
        else:
            ...
            NFA = utils_final.make_NFA_from_filename(NFA_filename)
            prev_pop.append(NFA)
    ...
    fitness_list = scoring_final.PCE_prediction(deltaHOMO, sum_oscs, prev_pop)
    ...
    with open('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/quick_analysis_data.csv', 'a') as quick_file:
    ...
    for x in range(len(fitness_list[0])):
        ...
        with open('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/full_analysis_data.csv', 'a') as analysis_file:
            analysis_file.write('%d,%s,%f,%f,%f,%s,\n' % (prev_gen, file_name, deltaHOMO, summed_oscs, PCE, donor))

    # make backup copies of output files
    shutil.copy('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/quick_analysis_data.csv', '/ihome/ghutchison/blp62/GA/running_GA/opt_GA/quick_analysis_data_copy.csv')
    shutil.copy('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/full_analysis_data.csv', '/ihome/ghutchison/blp62/GA/running_GA/opt_GA/full_analysis_data_copy.csv')
...
def next_gen(gen_counter, pop_size, smiles_list, fitness_list):
    ...
    setup = subprocess.call('mkdir /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s' % (gen_counter), shell=True)
    ...
    for parent in new_pop:
        parent_str = utils_final.make_NFA_str(parent, smiles_list)
        new_pop_str.append(parent_str)
        # make file name string w/ convention   
        file_name = utils_final.make_file_name(parent)
        duplicate_sTDDFT = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sequence/sTDDFT_output/%s.out /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/%s.out)' % (file_name, gen_counter, file_name), shell=True)
        duplicate_sTDDFT_prop = subprocess.call('(cp /ihome/ghutchison/blp62/GA/running_GA/sequence/sTDDFT_output/%s.prop /ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/%s.prop)' % (file_name, gen_counter, file_name), shell=True)
...
def main(gen_counter):
    ...
    smiles_list = utils_final.make_unit_list()
    ...
    if gen_counter == 1:
        ...
        rand_file = open('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/rand_states/randstate_%s.p' % (str(gen_counter)), 'wb')
    ...
    # run if NOT the initial generation
        ...
        open_rand = open('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/rand_states/randstate_%s.p' % (prev_gen), 'rb')
...
def check_prev_gen_complete(gen_counter):
    ...
    if gen_counter == 1:
        ...
        prop_counter = 0
        for x in glob.iglob('/ihome/ghutchison/blp62/GA/running_GA/opt_GA/generations/%s/*.prop' % (prev_gen)):
            prop_counter +=1
