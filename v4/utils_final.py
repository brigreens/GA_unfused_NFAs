...
def make_unit_list():
    ...
    with open("/ihome/ghutchison/blp62/GA/running_GA/opt_GA/unit_csvs/acc_core.csv", "r", encoding='utf-8') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        next(csv_reader)
        for row in csv_reader:
            acc_core.append(row[1])

    with open("/ihome/ghutchison/blp62/GA/running_GA/opt_GA/unit_csvs/acc_left_term.csv", "r", encoding='utf-8') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        next(csv_reader)
        for row in csv_reader:
            acc_term_L.append(row[1])

    with open("/ihome/ghutchison/blp62/GA/running_GA/opt_GA/unit_csvs/acc_right_term.csv", "r", encoding='utf-8') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        next(csv_reader)
        for row in csv_reader:
            acc_term_R.append(row[1])

    with open("/ihome/ghutchison/blp62/GA/running_GA/opt_GA/unit_csvs/don_core.csv", "r", encoding='utf-8') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        next(csv_reader)
        for row in csv_reader:
...
def make_NFA_str(temp_NFA, unit_list):
    ...
    if even_or_odd(int(sequence[0])) == 'even':
        NFA_string = unit_list[1][L_termindex]
    ...
    if even_or_odd(int(sequence[4])) == 'even':
        NFA_string = NFA_string + unit_list[1][R_termindex]
...
def make_seq_NFA(sequence, smiles_list):
    ...
    if even_or_odd(position_1) == 'even':
        L_term_index = random.randint(0, len(smiles_list[1])-1)
    # acceptor
    else:
    ...
    if even_or_odd(position_5) == 'even':
        if position_5 in temp_positions:
            ...
            R_term = random.randint(0, len(smiles_list[1])-1)
